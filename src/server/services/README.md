...
One service so far: `household.ts` (`ensureHousehold` — read-then-conditionally-call-`create_household`, backing the `household/bootstrap` route). The rest — `move_item`, `move_container`, `delete_container`, `redeem_invite` and the migrations they depend on — is Pair-session work (see the working agreement in `CLAUDE.md`).
